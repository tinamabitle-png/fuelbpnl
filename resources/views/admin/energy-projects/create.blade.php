...
@section('page-description', 'Create a renewable energy financing project')
@section('breadcrumb', 'Energy Projects / Create')

@section('content')
<div class="p-6 max-w-4xl">
    <form action="{{ route('admin.energy-projects.store') }}" method="POST" class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-6">
        @csrf
        ...
        <div>
            ...
            <input name="title" value="{{ old('title') }}" class="mt-2 w-full px-4 py-2 border border-gray-300 rounded-lg" required />
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <input type="date" name="start_date" value="{{ old('start_date') }}" class="mt-2 w-full px-4 py-2 border border-gray-300 rounded-lg" />
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" step="0.01" min="0" max="1000000000000" name="total_cost" value="{{ old('total_cost') }}" class="mt-2 w-full px-4 py-2 border border-gray-300 rounded-lg" />
            </div>
            <div>
                <label class="text-sm font-medium text-gray-700">Financed Amount (optional)</label>
                <input type="number" step="0.01" min="0" max="1000000000000" name="financed_amount" value="{{ old('financed_amount') }}" class="mt-2 w-full px-4 py-2 border border-gray-300 rounded-lg" />
            </div>
            <div>
                <label class="text-sm font-medium text-gray-700">Interest Rate (%)</label>
                <input type="number" step="0.01" min="0" max="60" name="interest_rate" value="{{ old('interest_rate') }}" class="mt-2 w-full px-4 py-2 border border-gray-300 rounded-lg" />
            </div>
            <div>
                <label class="text-sm font-medium text-gray-700">Term (months)</label>
                <input type="number" step="1" min="3" max="36" name="term_months" value="{{ old('term_months') }}" class="mt-2 w-full px-4 py-2 border border-gray-300 rounded-lg" />
            </div>
            <div>
                <label class="text-sm font-medium text-gray-700">Monthly Payment (optional)</label>
                <input type="number" step="0.01" min="0" max="1000000000000" name="monthly_payment" value="{{ old('monthly_payment') }}" class="mt-2 w-full px-4 py-2 border border-gray-300 rounded-lg" />
            </div>
            <div>
                <label class="text-sm font-medium text-gray-700">End Date</label>
                <input type="date" name="end_date" value="{{ old('end_date') }}" class="mt-2 w-full px-4 py-2 border border-gray-300 rounded-lg" />
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="activated_at" value="{{ old('activated_at') }}" class="mt-2 w-full px-4 py-2 border border-gray-300 rounded-lg" />
            </div>
            <div>
                <label class="text-sm font-medium text-gray-700">Completed At</label>
                <input type="date" name="completed_at" value="{{ old('completed_at') }}" class="mt-2 w-full px-4 py-2 border border-gray-300 rounded-lg" />
            </div>
        </div>
        ...
        <div class="flex gap-3">
            <button class="px-5 py-3 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-700">Save Project</button>
            <a href="{{ route('admin.energy-projects.index') }}" class="px-5 py-3 bg-white border border-gray-300 rounded-xl font-medium text-gray-700 hover:bg-gray-50">Cancel</a>
        </div>
    </form>
</div>
@endsection
