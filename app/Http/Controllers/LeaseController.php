...
class LeaseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.leases.index', compact('leases', 'stats', 'recentLeases', 'users'));
    }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'fuel_type' => 'nullable|in:petrol,diesel,premium',
            'liters' => 'nullable|numeric|min:1|max:1000',
            'fuel_price_per_liter' => 'nullable|numeric|min:1|max:100',
            'principal_amount' => 'required_without:liters|numeric|min:100|max:50000',
        ...
        try {
            ...
            if ($request->filled('liters') && $request->filled('fuel_price_per_liter')) {
                // Calculate from fuel parameters
                $principal = $request->liters * $request->fuel_price_per_liter;
            } else {
                // Use direct principal amount
            ...
            if ($user->available_credit < $totalAmount) {
                return redirect()->back()
                    ->with('error', 'User does not have sufficient credit limit. Available: ZAR ' . number_format($user->available_credit))
                    ->withInput();
            }
        ...
        try {
            ...
            if ($creditDifference > 0) {
                if ($lease->user->available_credit < $creditDifference) {
                    return redirect()->back()
                        ->with('error', 'User does not have sufficient credit limit for the increase')
                        ->withInput();
                }
        ...
        $callback = function() use ($leases) {
            ...
            foreach ($leases as $lease) {
                fputcsv($file, [
                    ...
                    $lease->fuel_type ?? 'N/A',
                    $lease->liters ?? 'N/A',
                    $lease->fuel_price_per_liter ?? 'N/A',
    ...
    /**
     * Calculate lease preview.
     */
    public function calculate(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'principal_amount' => 'nullable|numeric|min:100|max:50000',
            'liters' => 'nullable|numeric|min:1|max:1000',
            'fuel_price_per_liter' => 'nullable|numeric|min:1|max:100',
            'interest_rate' => 'required|numeric|min:0|max:100',
            'term_days' => 'required|integer|min:1|max:365',
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'calculation' => [
                        ...
                        'daily' => number_format($dailyRepayment, 2),
                        'interest_rate' => $interestRate . '%',
                        'term_days' => $request->term_days,
                    ]
                ]
